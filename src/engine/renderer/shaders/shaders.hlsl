...
float4 CompteVelocity(float4 position, float4 positionLast)
{
	float2 pos = position.xy / positionLast.w;
	float2 posLast = positionLast.xy / positionLast.w;
	
	float2 dif = (posLast - pos);
	dif *= 10.0f;
	return float4((dif +1.0f)*0.5f, 0.0f, 1.0f);
}
