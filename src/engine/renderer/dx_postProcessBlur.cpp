...
dx_postProcessBlur::dx_postProcessBlur()
{
}
...
void dx_postProcessBlur::InitShader(D3DShaders::D3D12ShaderCompilerInfo &shaderCompiler)
{
	// Load and compile the ray generation shader
	mPass.m_rgs = D3DShaders::RtProgram(D3DShaders::D3D12ShaderInfo(L"shaders\\BlurCS.hlsl", L"main", L"cs_6_0"));
...
void dx_postProcessBlur::InitPipeline()
{
...
void dx_postProcessBlur::Init_CBVSRVUAV_Heap()
{
	...
	HeapAddSrvArrayHelper(mDevice, handle, dx.dx_renderTargets->mNoiseImage.texture);							handle.ptr += handleIncrement;
	HeapAddSrvHelper(mDevice, handle, dx_renderTargets::POST_SPP_RT, DXGI_FORMAT_R32_UINT);				handle.ptr += handleIncrement;
}

void dx_postProcessBlur::Render()
{
	...
	dx.dx_renderTargets->SetRenderTargetTextureState(D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, dx_renderTargets::POST_SPP_RT);
	...
	barrier.pResource = dx.dx_renderTargets->GetRenderTargetTexture(dx_renderTargets::POST_Reproject_RT);
	transitionBarrier.UAV = barrier;
