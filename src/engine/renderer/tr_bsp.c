...
static void AddSurfaceToBottomLevelDxr(msurface_t *surf)
{
	//Note: Skip fog & sky. Could do a really cool version of fog with ray taycing
...
void AddGridToBottomLevelDxr(msurface_t *surf)
{
	...
	lodError = r_lodCurveError->value; //MICK, Hack fixed lod
...
void RE_LoadWorldMap(const char *name) {
...

	{
		...
		case MOD_BRUSH:
		{
			for (int j = 0; j < m->bmodel->numSurfaces; ++j)
			{
				msurface_t* sm = &m->bmodel->firstSurface[j];

				if (SF_FACE == *sm->data)
				{
					AddSurfaceToBottomLevelDxr(sm);
				}
