...
void R_Modellist_f(void) {
	...
	for (i = 1; i < tr.numModels; i++) {
		...
		ri.Printf(PRINT_ALL, "dataSize=%8i : lods=(%i) name=%s", mod->dataSize, lods, mod->name);

		switch (mod->type)
		{
			case MOD_BAD:
			{
				ri.Printf(PRINT_ALL, ",type=MOD_BAD\n");
				break;
			}
			case MOD_BRUSH:
			{
				ri.Printf(PRINT_ALL, ",type=MOD_BRUSH,numSurfaces=%i\n", mod->bmodel->numSurfaces);
				break;
			}

			case MOD_MESH:
			{
				ri.Printf(PRINT_ALL, ",type=MOD_MESH\n");
				break;
			}

			case MOD_MD4:
			{
				ri.Printf(PRINT_ALL, ",type=MOD_MD4\n");
				break;
			}
		}
		total += mod->dataSize;
